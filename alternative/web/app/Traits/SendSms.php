...
trait SendSms {
    ...
    if($smsby->status==1){       
     ...
     public function otpmsg($otpval,$user_phone) 
     {
         ...
         $getInvitationMsg = "Your OTP is : ".$otpval.". Note: Please DO NOT SHARE this OTP with anyone."; 
 ...
 public function sendrejectmsg($cause,$user,$cart_id) {
    ...
    if($smsby->status==1){       
     ...
     public function rechargesms($user_name, $add_to_wallet,$user_phone) {
        $getInvitationMsg = "Hey ".$user_name." :your wallet recharge of ".$add_to_wallet. " is successful.";
        $smsby =  DB::table('smsby')
               ->first();
    if($smsby->status==1){       
        if($smsby->msg91==1){       
