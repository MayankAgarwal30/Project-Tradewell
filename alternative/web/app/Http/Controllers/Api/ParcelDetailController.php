...
->insert([
 ...
 public function parcel_orderplaced(Request $request)
    ...
    public function cancel_order_parcel(Request $request)
    {
      $created_at = Carbon::now();     
      $cart_id = $request->cart_id;
       $user = DB::table('parcel_details')
              ->where('cart_id',$cart_id)
              ->first();
      ...
      $order = DB::table('parcel_details')
       ...
       if($order){
        if($user->payment_method == 'COD' || $user->payment_method == 'Cod' || $user->payment_method == 'cod'){
            $newbal1 = $userwa1->wallet_credits + $user->wallet;  
        ...
        elseif($user->payment_status=='success'){
                  $newbal1 = $userwa1->wallet_credits + $user->rem_price + $user->wallet;
                  
                  $insert= DB::table('wallet_history')
                    ->insert([
                                'type'=>'Refund',
                                'amount'=>$user->wallet +$user->rem_price,
                                'user_id'=>$user_id1,
                                'created_at'=>$created_at,
                             ]);
              }
