 ...
 public function pharmacy_user_completed_orders(Request $request)
    {
      ...
      $order = DB::table('order_details')
        ...
        $data[]=array('order_status'=>$completed->order_status, 'delivery_date'=>$completed->delivery_date,'time_slot'=>$completed->time_slot,'payment_method'=>$completed->payment_method,'payment_status'=>$completed->payment_status,'paid_by_wallet'=>$completed->paid_by_wallet, 'cart_id'=>$completed->cart_id ,'price'=>$completed->total_price,'del_charge'=>$completed->delivery_charge,'remaining_amount'=>$completed->rem_price,'coupon_discount'=>$completed->coupon_discount,'delivery_boy_name'=>$completed->delivery_boy_name,'delivery_boy_phone'=>$completed->delivery_boy_phone,'vendor_name'=>$completed->vendor_name,  'address'=>$completed->address,'vendor_loc'=>$completed->vendor_loc,'data'=>$order); 
        }
        }
        else{
            $data=array('data'=>[]);
   ...
   public function pharmacy_user_ongoing_order(Request $request)
      ...
      public function dealproduct(Request $request)
    {
        ...
        $deal_p= DB::table('deal_product')
         ...
         public function order_generate_by_pharmacy(Request $request)
    {   
        $current = Carbon::now();
        $data= $request->order_array;
        $data_array = json_decode($data);
